...
my $full_debug_logfile = 0;		# hack this line to enable this
if ($full_debug_logfile) {
  $dbg = 1; close STDERR; open (STDERR, ">".$ENV{HOME}."/.mhthread.log");
}
...
if ($full_debug_logfile) {
  if (open (IN, "<$ENV{HOME}/Mail/inbox/".$mh_sequences_file_name)) {
    while (<IN>) { warn "newseqfile: $_"; } close IN;
  }
}
...
use vars qw(@to_unlink @to_rename %num2seq %newseqs %changedseqs);
...
sub mh_rewrite_folder {
  ...
  if (open (IN, "<".$folder."/".$mh_sequences_file_name)) {
    while (<IN>) {
      ...
      $dbg and warn "debug: old sequences: seq=$seq nums=$msgs\n";
  ...
  if ($changed) {
    ...
    {
      if (!$changedseqs{$seq}) {
	$dbg and warn "debug: new sequences: seq=$seq unchanged\n";
	next;
      }
      ...
      $dbg and warn "debug: new sequences: seq=$seq cmd=".join (' ', @cmd)."\n";
      system @cmd;
      if ($? >> 8 != 0) {
...
sub mh_rewrite_reorder_message {
  ...
  if ($oldnum eq $newnum && $oldprefix eq $newprefix) {
    ...
    foreach my $seq (@{$num2seq{$oldnum}}) {
      $dbg and warn "debug: $oldnum: will keep seq $seq\n";
      push (@{$newseqs{$seq}}, $oldnum);
    }
    return 0;
  }
  ...
  foreach my $seq (@{$num2seq{$oldnum}}) {
    $dbg and warn "debug: $oldnum->$newnum: will fix seq $seq\n";
