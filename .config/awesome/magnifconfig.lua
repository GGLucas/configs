...
util = {
    ...
    dropdown = function (prog, screen, size, position, id)
        if screen == nil then screen = capi.mouse.screen end
        if size == nil then size = 0.2 end
        if position == nil then position = "top" end
        if id == nil then id = prog end
        ...
        if not dropdown[id] then
            ...
            awful.hooks.unmanage.register(function (c)
                for scr, cl in pairs(dropdown[id]) do
                    if cl == c then
                        dropdown[id][scr] = nil
                    end
                end
        ...
        if not dropdown[id][screen] then
            spawnw = function (c)
                -- Store client
                dropdown[id][screen] = c
                ...
                if size < 1 then
                    if position == "top" or position == "bottom" then
                        size = screengeom.height*size
                    else
                        size = screengeom.width*size
                    end
                end
                ...
                elseif position == "right" then
                    c:geometry({
                        x = screengeom.x+(screengeom.width-size),
                        y = screengeom.y,
                        width = size,
