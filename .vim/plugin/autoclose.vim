...
fun <SID>ToggleAutoCloseMappings() " --- {{{2
    if g:autoclose_on
        ...
        inoremap <silent> <CR> <C-R>=<SID>AttemptIndent()<CR>
...
function <SID>OpenSpecial(ochar,cchar) " ---{{{2
    ...
    if a:ochar == line[(col)] && a:cchar == line[(col+1)] "&& strlen(line) - (col) == 2
        ...
        endwhile
        return "\<Delete>"
    elseif a:ochar == line[(col-1)] && line[(col)] == " " && a:cchar == line[(col+1)]
        return "\<Left>\<Delete>".a:ochar.<SID>CloseStackPush(a:cchar)
    endif
...
function <SID>CloseStackPop(char) " ---{{{2
    ...
    while ((lastpop == '' && popped == '') || lastpop != a:char)
